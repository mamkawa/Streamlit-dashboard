...
if df is not None:
    ...
    selected_year = st.sidebar.selectbox("年", years)
    
    months = sorted(df[df['年'] == selected_year]['月'].unique())
    selected_month = st.sidebar.selectbox("月", months)
    
    days = sorted(df[(df['年'] == selected_year) & (df['月'] == selected_month)]['日'].unique())
    selected_day = st.sidebar.selectbox("日", days)
    ...
    if analysis_type == "データ概要":
        ...
        st.write("選択されたデータの期間:")
        st.write(f"開始日: {filtered_df['購入日'].min().strftime('%Y年%m月%d日')}")
        st.write(f"終了日: {filtered_df['購入日'].max().strftime('%Y年%m月%d日')}")
        
        # 選択されたデータの概要
        st.subheader("フィルター適用後のデータ概要")
        st.write(f"データ件数: {len(filtered_df):,}件")
        st.write(f"合計売上金額: ¥{filtered_df['売上金額'].sum():,.0f}")
        st.write(f"平均売上金額: ¥{filtered_df['売上金額'].mean():,.0f}")
